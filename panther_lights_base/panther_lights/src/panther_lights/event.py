...
class Event:
    ...
    def __init__(self, event_yaml, num_led, global_brightness):
        '''event combining front and tail animation'''
        ...
        self._tail_animation_runtime = None
        ...
        if 'both' in event_yaml['animation'].keys():
            ...
            self._tail_animation = copy.deepcopy(self._front_animation)
            self._tail_animation.panel = 1
            ...
            self._tail_animation = self._match_animation_type(
                event_yaml['animation']['tail'], num_led, global_brightness, 1)
        else:
            missing_animations = (set(["front", "tail", "both"])) - set(event_yaml["animation"].keys())
    ...
    def percent_done(self):
        ...
        return min(self._front_animation_runtime.percent_done, self._tail_animation_runtime.percent_done)
    ...
    def finished(self):
        ...
        return self._front_animation_runtime.finished and self._tail_animation_runtime.finished

    @property
    def spawned(self):
        '''returns if both animations were already spawned'''
        return self._front_animation_runtime is not None and self._tail_animation_runtime is not None